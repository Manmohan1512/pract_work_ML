...
# MAGIC %md
# MAGIC ### Langchain Blogs- Seperate Vector Store
# MAGIC
...
def agent(state):
    ...
    #ag_model = ChatGroq(model="qwen-qwq-32b")
    print('passing the GROQ key')
    GROQ_API_KEY="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"
    model= ChatGroq(model= "openai/gpt-oss-20b",groq_api_key=GROQ_API_KEY )
    model = model.bind_tools(tools)
...
graph.invoke({"messages": "help me more in langchain?"})

# COMMAND ----------

graph.invoke({"messages": "add 2 and 6"})